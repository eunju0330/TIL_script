...
console.log(itemLoad, startIndex, endIndex); //4 0 4
console.log(container);

loadMoreContent() //이벤트없이 쓴 이유 - 처음보이는 제품나열을 위한, 함수 호출
function loadMoreContent(){
    ...
    for(let i=startIndex; i<endIndex; i++){
        const newLi = document.createElement('li')//ul에 삽입해야함. ul에는 li만 삽입 할 수 있다.
        newLi.innerHTML = `<img src="${kurlyDB[i].src}" alt="${kurlyDB[i].name}">`; //이미지 태그 쓸거니까 innerHTML'
        ...
        newLi.innerHTML += `<p class="price">${Math.round((kurlyDB[i].price * (1 - kurlyDB[i].sale / 100))).toLocaleString('ko-kr')}</p>`;
