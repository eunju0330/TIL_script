...
// 2. 우측 ... 메뉴 클릭 시 저장~신고 등 기능 실행 (o)-저장~신고 제작필요
...
//(아래) likeImg객체가 a태그로 클릭 시 새로고침돼서 스크롤이 위로 올라가는 문제 해결을 위한 방법
//콜백함수 호출이 아닌 익명함수 또는 화살표함수 이용필수
//익명함수 또는 화살표함수로 작성 시 이벤트 앞 객체정보가 함수의 매개변수로 자동으로 대입된다.
//위 매개변수가 받은 정보를 확인하려면 매개변수에 변수명(e:이벤트의 약자)을 작성하고 console.log()로 확인할 수 있다.
...
// 6. 댓글 아이콘 터치 시 댓글 입력창 실행(o)-제작필요, 실행화면에서 바깥쪽 영역 터치하면 댓글 입력창 닫힘(o)
// 7. 공유 아이콘 터치 시 공유가능 계정들 표시된 창 실행(o)-제작필요, 바깥쪽 영역 터치 시 닫힘(o)
// 8. 즐겨찾기 아이콘 터치 시 즐겨찾기 저장(x-DB필요)이 됨과 동시에 컬렉션 선택화면 출력(o), 바깥쪽 영역 터치 시 닫힘(o)
...
//동적기능 예) 해외 IP차단, 동일 IP접속차단, 성인사이트접속인증, 로그인에 따른 접속가능 페이지일 경우 로그인 유무 확인 등
// 11. 날짜 터치 기능 없음(x js)
